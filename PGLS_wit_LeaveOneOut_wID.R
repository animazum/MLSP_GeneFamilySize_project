...
Bootstrap_LOO_PGLS <- function(traits, pheno.col.nums, tree, spp.col.num, gene.numbers, No.variables, where.Save.it, n_bootstrap = 100) {
  ...
  for (i in 1:n_bootstrap) {
    ...
    model_success <- FALSE
    error_msg <- NA
    # IDName<- paste(".LOOremoved",Removed_Species,".", sep = "")
    tryCatch({
      PGLS.GF.WithID(
        traits = traits_sub,
...
results_Bootstrap_LOO.230 <- Bootstrap_LOO_PGLS(
  ...
  No.variables = 2, 
  where.Save.it = path1, 
  n_bootstrap = 230 # Number of iterations
)
table(results_Bootstrap_LOO[,c(2,3)])