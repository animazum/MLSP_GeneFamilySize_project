...
z_scores <- (traits.filtered$Maximum.longevity..yrs. - mean(traits.filtered$Maximum.longevity..yrs.)) / sd(traits.filtered$Maximum.longevity..yrs.)
outliers <- which(abs(z_scores) > 3)  # Identify outliers beyond 3 SDs
...
PGLS.GF.size(traits = traits.filtered, pheno.col.nums = c(30), No.variables = 1, 
