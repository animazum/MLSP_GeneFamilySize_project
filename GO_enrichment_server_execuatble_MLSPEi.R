...
for(bd in 1:length(BG_genelists)){
  #rm(list=ls())
  ...
  databaseName<-BG_genelists[bd]
  infile<-BG_genelists[[bd]]
  head(infile, 3)
  colnames(infile)
  ...
  GOenrichment<-function(filename1, BPFilt, PHENO, DATABASE, METODO, focaltrait,FAMIDCol, focusGenes, nameFile) { 
    ...
    if (DATABASE == "Ensembl") {
      ...
      GO.TERMS <-"/datos/animazun/MLSP/GO_enrichment_databases/GO_terms/GOids_to_EnsemblID_v.ensembl113_human_short.csv" ### GO terms file to be used
  ...
  GOenrichment(filename1 = infile, BPFilt = 200, PHENO = 2, focusGenes = 2,
               DATABASE = "Ensembl", METODO = "fdr", FAMIDCol = 1,focaltrait = "MLSP_Ei", nameFile = names(databaseName))
